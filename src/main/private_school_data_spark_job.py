...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: private_school_data_spark_job.py <hostname> <port> <input_path> <output_path>", file=sys.stderr)
        sys.exit(-1)
    ...
    spark = SparkSession.builder.master(hdfs).appName("ProcessPrivateSchoolData").getOrCreate()

    # Process Public School Data and Output To HDFS
    processData(spark, f"{input_path}/private_school_data", f"{output_path}/private_school_out")
